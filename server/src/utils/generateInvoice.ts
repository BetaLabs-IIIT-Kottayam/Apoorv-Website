...
const generateInvoice = (order: PopulatedOrder) => {
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(`Date: ${new Date(order.paidAt!).toLocaleDateString()}`, margin + 10, 75);
  ...
  order.items.forEach((item, index) => {
      ...
      const nameLines = doc.splitTextToSize(item.merchId.name, columnWidths[0] - 5);
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFont('helvetica', 'bold');
  doc.text(`Total: Rs.${order.totalAmount}`, pageWidth - margin - 75, totalBoxY + 13);

  // Add footer
  doc.setTextColor(128, 128, 128);
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text('Thank you for shopping with APOORV!', pageWidth/2, pageHeight - 20, { align: 'center' });
